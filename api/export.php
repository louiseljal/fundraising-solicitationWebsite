...
if ($report === 'members') {
    ...
    $stmt = $pdo->query("
        ...
        LEFT JOIN donations d ON u.user_id = d.user_id AND d.payment_status = 'Completed' AND (d.is_deleted = 0 OR d.is_deleted IS NULL)
...
if ($report === 'analytics') {
    ...
    if (!file_exists($env_path)) {
        ...
        $dw_db   = $env_vars['DW_NAME'] ?? 'fundraising_dw';
        $dw_user = $env_vars['DW_USER'] ?? 'root';
    ...
    try {
        ...
        // Fallback to operational database if data warehouse is unavailable
        try {
            require_once dirname(__DIR__) . '/includes/db.php';
            $dw_pdo = $pdo;
            // Use operational database for analytics if DW is not available
            $title   = 'OLTP Monthly Summary (Fallback)';
            ...
            die('Both Data Warehouse and operational database are currently offline.');
        }
    }
